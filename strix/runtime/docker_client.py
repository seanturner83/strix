...
class StrixDockerSandboxClient(DockerSandboxClient):
    ...
    async def delete(self, session: SandboxSession) -> SandboxSession:
        ...
        if container_id:
            ...
            # ConnectionError, a sibling of APIError under RequestException,
            # not a subclass — so it escapes an APIError-only suppress). This
            # fires on every credential-boundary teardown in the CI resume
            # loop, where the socket is already going away.
